...
class Conductor:
  ...
  def run(self, objective, examples =""):
    task_creator_model = ChatOpenAI(temperature=0.9,model_name=self.model_name, openai_api_key=self.openai_api_key)
    chain = LLMChain(llm=task_creator_model, prompt=get_new_prompt(), verbose=self.verbose)
    return json.loads(chain.run(objective=objective, examples = examples))