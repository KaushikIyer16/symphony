...
# Provided data for creating objectives and task list files
data_examples = [
    {
...
""")
  ...
  {
    "objective": "I want to research John Deere",
    "taskList": dedent("""
[
    {"id": 1,"task":"Look up John Deere","tool":"web-search","dependent_task_ids":[],"status":"incomplete", "result": null, "result_summary": null},
    {"id": 2,"task":"Summarize Company Details","tool":"text-completion","dependent_task_ids":[1],"status":"incomplete", "result": null, "result_summary": null},
    {"id": 3,"task":"Find the latest news about John Deere","tool":"web-search","dependent_task_ids":[],"status":"incomplete", "result": null, "result_summary": null},
    {"id": 4,"task":"Summarize the latest news","tool":"text-completion","dependent_task_ids":[3],"status":"incomplete", "result": null, "result_summary": null}
]
""")
  }
]
...
new_objectives_file_path = 'storage/objectives.txt'
new_task_lists_file_path = 'storage/task_lists.txt'

# Write objectives to the new file
with open(new_objectives_file_path, 'w', encoding='utf-8') as file:
    for index, item in enumerate(data_examples, start=1):
        file.write(f'Objective{index}: {item["objective"]}\n')

# Reformat and write task lists to the new file
with open(new_task_lists_file_path, 'w', encoding='utf-8') as file:
    for index, item in enumerate(data_examples, start=1):
        tasks_json = json.loads(item["taskList"])
        value = json.dumps(tasks_json)
        file.write(f'Objective{index}: {value}\n')

new_objectives_file_path, new_task_lists_file_path
